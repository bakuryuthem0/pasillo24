...
jQuery(document).ready(function($) {
	$('.btn-filtralo').on('click', function(event) {
		...
		{
			if ($('.to-filter').hasClass('busq')) {
				window.location.replace('http://localhost/pasillo24/inicio/buscar?busq='+$('.to-filter').val());			
			}else
			{
				window.location.replace('http://localhost/pasillo24/publicaciones/categorias/'+$('.to-filter').val());
			}
		}
	});
