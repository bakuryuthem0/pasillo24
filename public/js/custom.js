...
jQuery(document).ready(function($) {
	$('.btnEliminarPub').click(function(event) {
		...
		$('.btnElimPublicacion').click(function(event) {
			$.ajax({
				url: 'http://pasillo24.com/eliminar/publicacion',
				type: 'POST',
				dataType: 'json',
				data: {'id': $(this).val()},
				beforeSend:function(){
...
$(document).ready(function() {
	$('.btn-responder').click(function(event) {
		...
		$('.enviarRespuesta').click(function(event) {
			...
			$.ajax({
				url: 'http://pasillo24.com/publicacion/comentarios/respuesta',
				type: 'POST',
				dataType: 'json',
				data: datos,
				beforeSend:function()
...
jQuery(document).ready(function($) {
	$('.sendPubValue').on('click',function(event) {
		...
		$('.sendValueSeller').click(function(event) {
			...
			var neg = $('#neg'),pos = $('#pos');
			$.ajax({
				url: 'http://pasillo24.com/usuario/valorar-vendedor',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				beforeSend:function()
				{
					...
					$('#neg').animate({
						...
						'opacity': 0},
						50, function() {
							$(this).after('<img src="http://pasillo24.com/images/loading.gif" style="width:25px;height:25px;display:inline-block;" class="loading">')
							$(this).css({'display':'none'});
					});
				},
				success:function(response)
...
jQuery(document).ready(function($) {
	$('.sendPubValue').on('click',function(event) {
		...
		$('.sendValueBuyer').click(function(event) {
			...
			$.ajax({
				url: 'http://pasillo24.com/usuario/valorar-comprador',
				type: 'POST',
				dataType: 'json',
				data: dataPost,
				beforeSend:function()
				{
					...
					$('#neg').animate({
						...
						'opacity': 0},
						50, function() {
							$(this).after('<img src="http://pasillo24.com/images/loading.gif" style="width:25px;height:25px;" class="loading">')
							$(this).css({'display':'none'});;
					});
				},
				success:function(response)
